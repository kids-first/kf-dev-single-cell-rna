...
params:
  ...
  max_genes: 4000 # maximum number of genes per cell; it was 6000
  max_mt: 5  # maximum percent mitochondrial reads per cell; it was 15
...
knitr::opts_chunk$set(echo = FALSE,
                      warning = FALSE, 
                      message = FALSE, 
                      fig.path = file.path(plots_dir, paste0("Create-", sample_name, "/")),
                      fig.height = 10, 
                      fig.width = 15,
                      dev = c("pdf"))
```
...
library(ggpmisc)
library(ggrepel)
load_all("/Users/chronia/CHOP/GitHub/scooter") # PATH TO SCOOTER
theme_set(theme_bw())
...
data_path: "/Users/chronia/CHOP/projects/HOPE/HOPE-analysis/analyses/01_snRNA-seq/00-alignment/7316-2594_include_introns/outs/filtered_feature_bc_matrix/" # path to the directory with the outs file
...
results_dir <-
  file.path(analysis_dir, "results", paste0("Create-", sample_name, "/"))
...
yaml::write_yaml(params, glue("{results_dir}/params.yml"))
log_file = file.path(results_dir, "log")
```
...
```
The seurat object  was saved as `r paste0(results_dir, "/", "seurat_obj_raw.rds")`
...
if(!is.null(hashtag_ID) | !is.null(hashtag_file)) {
  ...
  saveRDS(seurat_obj, file = paste0(results_dir, "/", "seurat_obj_doublet.rds"))
...
seurat_obj <- subset(seurat_obj, subset = nFeature_RNA > 0 & nFeature_RNA < 6000 & pct_mito < 5)
...
Save metadata as a csv to `r glue("{results_dir}/metadata_create_{sample_name}.csv")`
...
dir.create(file.path(results_dir, "scratch"), showWarnings = FALSE)

for(i in 1:nrow(plot_dr)){
  print(current_plot <- plot_scatter(metadata = metadata,
                               results_dir = file.path(results_dir, "scratch"),
...
Save rds to `r paste0(results_dir, "/", "seurat_obj.rds")`
