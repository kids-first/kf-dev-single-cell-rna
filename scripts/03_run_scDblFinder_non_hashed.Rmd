---
title: "ScDblFinder filtering with recoverDoublets for non-hashed experiements - `r params$sample_name`"
...
output:
  html_document:
    ...
    df_print: paged
params:
  data_path: "/gpfs/data/aifantislab/home/chrona01/projects/WT1/analysis/01_RNA/data/WT1_non_hashed" # path to the directory with the raw Seurat object
  out_path: "/gpfs/data/aifantislab/home/chrona01/projects/WT1/analysis/01_RNA/data/scDblFinder_non_hashed/" # path to the directory with the output files
  log_file: NULL
---
...
# read dir where seurat_obj_raw.rds was saved during the Seurat_QC step
dirs <- dir(path=data_path,  pattern = "seurat_obj_raw.rds", full.names=TRUE, recursive=TRUE)
```
...
```{r save_seurat_obj}
saveRDS(seurat_obj, paste0(out_path, "seurat_obj_sce.rds"))
```

# Run scDblFinder on seurat_obj
We are running the scDblFinder function from the scDblFinder package to identify cells that are doublets based on 
cluster-based generation of artificial doublets. 
...
# create library dir
sample_name <- gsub("Create-", "", str_split_fixed(dirs,"/",12)[,11])
...
print(table(seurat_obj$scDblFinder.class)[2]/(table(seurat_obj$scDblFinder.class)[1]+table(seurat_obj$scDblFinder.class)[2])*100)

hashed.doublets <-seurat_obj@colData[,str_detect(colnames(seurat_obj@colData),"scDblFinder")] %>% as.data.frame() %>% tibble::rownames_to_column("cell")
...
plotUMAP(seurat_obj, colour_by="scDblFinder.cluster", point_size = 0.1) + ggtitle("Clusters") ,
...
saveRDS(seurat_obj, paste0(temp_path, "sce_obj_", sample_name,".rds") )
