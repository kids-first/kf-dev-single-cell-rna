...
Rscript -e "rmarkdown::render('scDblFinder.Rmd', clean = TRUE,
      params=list(results_dir = '/Users/chronia/CHOP/GitHub/kf-dev-single-cell-rna/scripts/example/', 
                  data_path = '/Users/chronia/CHOP/GitHub/kf-dev-single-cell-rna/scripts/example/scDblFinder-test/seurat_obj_with_doublets_test.rds',
                  sample_name = 'test'))"
```
...
doublets_number <- singlets_doublets_number %>% 
  filter(prediction == "doublet") %>% 
  select(count) %>% sapply(as.character) %>% as.vector
